...
router.post('/', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (verifiedOffer?.expires_at && Date.parse(verifiedOffer.expires_at) < Date.now()) {
    ...
    // Price integrity check — detect any meaningful difference
    const currentPrice   = parseFloat(verifiedOffer?.total_amount || 0);
    const requestedPrice = parseFloat(total_amount || 0);
    const diff = Math.abs(currentPrice - requestedPrice);
    if (currentPrice > 0 && diff > 1) {
  ...
  const orderBody = {
    ...
    payments: [{
      type:     'balance',
      amount:   String(verifiedOffer?.total_amount || total_amount),
      currency: verifiedOffer?.total_currency     || currency,
    }],
