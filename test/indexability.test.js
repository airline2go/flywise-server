...
describe('hasVerifiedFlightEvidence (canonical policy)', () => {
  ...
  test('airline_count alone is NOT evidence', () => {
...
describe('getRouteIndexabilityDecision — enforced vs legacy (shared fixture)', () => {
  ...
  test('explicit legacy mode preserves legacy behaviour', () => {
    expect(getRouteIndexabilityDecision({ distance_km: 500 }, { enforce: false }).indexable).toBe(true);
  });
...
describe('routeIndexable', () => {
  ...
  test('strict policy requires verified evidence; airline count alone does not', () => {
...
describe('buildConnectivity', () => {
  ...
  const c = buildConnectivity(routes, { enforce: false });
