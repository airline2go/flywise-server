const { hasVerifiedFlightEvidence, getRouteIndexabilityDecision, buildConnectivity } = require('../src/services/indexability');
...
describe('route evidence policy', () => {
  test('strict evidence is enabled by default', () => {
    ...
    if (previous == null) delete process.env.SEO_EVIDENCE_POLICY_ENFORCED; else process.env.SEO_EVIDENCE_POLICY_ENFORCED = previous;
  });
  test('airline count alone is not flight evidence, even when integer', () => {
