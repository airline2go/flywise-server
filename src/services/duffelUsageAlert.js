...
async function shouldAlert(alertKey) {
  ...
  if (redis && redis.status === 'ready') {
    try { return (await redis.set(key, '1', 'EX', 3600, 'NX')) === 'OK'; } catch (_) {}
  }
  const now = Date.now();
...
async function sendAlert(subject, html) {
  ...
  try {
    ...
    log('info', 'duffel_usage_alert_sent', { subject });
    return true;
  } catch (e) {
...
async function recordDuffelAttemptAlert({ source, trigger, endpoint }) {
  ...
  if (hour !== HOURLY_THRESHOLD && day !== DAILY_THRESHOLD) return;

  const period = hour === HOURLY_THRESHOLD ? 'hour' : 'day';
  const count = period === 'hour' ? hour : day;
  if (!(await shouldAlert(`${period}:${utcKey(period)}`))) return;
  await sendAlert(
