...
// - when configured, a Cloudflare edge secret closes the direct-to-Render bypass
...
function shield(app) {
  ...
  app.use((req, res, next) => {
    ...
    // When configured, only requests carrying the Cloudflare-injected secret
    // may reach protected API routes. This runs before browser provenance/auth
    // so a valid token cannot bypass the origin boundary.
