...
module.exports = (app) => app.get('/sitemap-data/routes-localized',rateLimit('content',2500,60000),async(req,res)=>{try{
  ...
  // Keep the second PostgREST `.in(id, ids)` query bounded. A 1000-row locale
  // page can create an oversized request URL once hundreds of UUIDs are encoded;
  // 200 keeps the query comfortably below proxy/request-line limits while still
  // paging the full locale catalogue deterministically.
  ...
  const items=rows.map(r=>{
    ...
    return {id:route.slug,language,lastmod:lastmod(r.updated_at,r.seo_generated_at,route.updated_at,route.insights_updated_at,route.created_at)};
  }).filter(Boolean);
