  ...
  app.get('/sitemap-data/routes', limit, async (req, res) => {
    try {
      ...
      const { data, error } = await supa.from('route_pages')
        .select('slug,updated_at,insights_updated_at,created_at,distance_km,avg_duration_min,airline_count,route_score,weekly_flights,stop_distribution,price_sample_count,itinerary_count,intro_text,custom_faq,origin_iata,destination_iata')
        .eq('status', 'published')
